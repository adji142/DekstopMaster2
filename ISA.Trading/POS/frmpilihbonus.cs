...
{
    public partial class frmpilihbonus : ISA.Trading.BaseForm
    {
        string idToko, barangid;
        int SumqtyBarang, SumqtyKelompok, qtystokakhir, i, qtybonusfinal;
        DataTable dtsumKelipatanQtyBarang, dtsumKelipatanQtyKelompok,dtPromoBengkelServiceOli;
        double SumHargaBarang, SumHargaKelompok;
        Guid rowidpromodetail;
        DataTable dt;
        string PaketPromoBkl = "";
        ...
        //public frmpilihbonus(Form caller, string TokoID, int sumqtybarang, double sumhargabarang, int sumqtykelompok, 
        //                     double sumhargakelompok, DataTable dtPromoBarangBonus, DataTable dtpromoKelompok, 
        //                     string PaketPromoService)

        public frmpilihbonus(Form caller, 
                             string TokoID, 
                             int sumqtybarang, 
                             double sumhargabarang, 
                             int sumqtykelompok, 
                             double sumhargakelompok, 
                             DataTable dtPromoBarangBonus, 
                             DataTable dtpromoKelompok, 
                             DataTable dtPromoBengkel,
                             string _paketService)
        {
            InitializeComponent();
            this.Caller = caller;
            idToko = TokoID;
            ...
            dtPromoBengkelServiceOli = dtPromoBengkel;
            PaketPromoBkl = _paketService;
        ...
        public void PromoBengkel()
        {
            DataTable dtPrmBengkel = Prolib.DataPromoBengkel(SumqtyKelompok, SumHargaKelompok, "");
            datagridviewpromotetap.DataSource = dtPrmBengkel.DefaultView;
            for (int i = 0; i < datagridviewpromotetap.Rows.Count; i++)
            {
                barangid = dtPrmBengkel.Rows[i]["id_brg"].ToString();
                cekstok(barangid);
                int qtybonus = Convert.ToInt32(dtPrmBengkel.Rows[i]["QtyPromo"]);
                if (qtybonus > qtystokakhir)
                {
                    qtybonusfinal = qtystokakhir;
                }
        ...
        public void PromoBengkelServiceOli()
        {
            DataTable dtPrmBengkel = dtPromoBengkelServiceOli;
            datagridviewpromotetap.DataSource = dtPrmBengkel.DefaultView;
            for (int i = 0; i < datagridviewpromotetap.Rows.Count; i++)
            {
                barangid = dtPrmBengkel.Rows[i]["id_brg"].ToString();
        ...
        private void datagridviewpromotetap_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (ix >= 0)
            {
                DataGridViewCheckBoxCell checkrow = (DataGridViewCheckBoxCell)datagridviewpromotetap.Rows[ix].Cells["pilihTetap"];
                ...
                if (isChecked)
                {
                    ...
                    for (int i = 0; i < datagridviewpromotetap.Rows.Count; i++)
                    {
                        if (i != datagridviewpromotetap.Rows.Count)
                        {
                            ((DataGridViewCheckBoxCell)datagridviewpromotetap.Rows[i].Cells["pilihTetap"]).Value = false;
                        }
                    }
                }
                datagridviewpromotetap.Invalidate();
            }


            //if (datagridviewpromotetap.Columns[e.ColumnIndex].Name == "pilihTetap")
            //{
            //    DataGridViewCheckBoxCell checkCell = (DataGridViewCheckBoxCell)datagridviewpromotetap.Rows[e.RowIndex].Cells["pilihTetap"];
