...
namespace ISA.Utility
{
    ...
    public class JSON
    {
        ...
        static public JSON Object
        {
            get
            {
                return new JSON(JSONType.Object);
            }
        }
        static public JSON Array
        {
            get
            {
                return new JSON(JSONType.Array);
            }
        }
        static public JSON Number
        {
            get
            {
                return new JSON(JSONType.Number);
            }
        }
        static public JSON String
        {
            get
            {
                return new JSON(JSONType.String);
            }
        }
        static public JSON Bool
        {
            get
            {
                return new JSON(JSONType.Bool);
            }
        }
        static public JSON Unknown
        {
            get
            {
                return new JSON(JSONType.Unknown);
            }
        }
        #endregion
            ...
            while (len > i)
            {
                ...
                if (quote == 1)
                {
                    ...
                    else
                    {
                        ...
                        if (scr[ii - 1] == '\\' && (ii < 2 || (ii > 2 && scr[ii - 2] != '\\')))
                        {
                            i += 1;
                            cur += '"';
                            continue;
        ...
        static public JSON ParseFromURLData(string scr)
        {
            ...
            foreach (string c in ls)
            {
                ...
                else if (ls2.Length == 1 && ls2.First().Length > 0) dat.ObjAdd(Uri.UnescapeDataString(ls2.First()), JSON.Null);
            }
            return dat;
        }
        ...
        {
            if (opt == null) opt = JSON.Object;
            else if (opt.Type != JSONType.Object) opt = JSON.Object;
        ...
        {
            return ParseFromDataTable(dtbl, JSON.Null);
        }
        static public JSON ParseFromDataTable(DataTable dtbl, JSON opt)
        {
            ...
            JSON jdat = JSON.Array;
            foreach (DataRow dr in dtbl.Rows)
            {
                JSON sdat = JSON.Object;
        ...
        {
            return ParseFromDataRow(drow, JSON.Null);
        }
        ...
        static public class Tools
        {
            ...
            /**
             * Exporting big data to JSON file instantly
             */
            static public bool ExportDataTableToJSON(DataTable dtbl, JSON opt, string fpath)
            {
                if (opt == null) opt = JSON.Object;
                else if (opt.Type != JSONType.Object) opt = JSON.Object;

                using (StreamWriter strx = new StreamWriter(fpath, false, System.Text.Encoding.UTF8))
                {
                    if (dtbl != null && dtbl.Rows.Count > 0)
                    {
                        strx.Write("[");
            ...
            /*
             * Exporting big data to XML file instantly
             */
            static public bool ExportDataTableToXML(DataTable dtbl, string fpath)
            {
                ...
                using (XmlWriter wt = XmlWriter.Create(fpath, stg))
                {
                    wt.WriteStartDocument();
                    wt.WriteStartElement("NewDataSet");
                    foreach (DataRow row in dtbl.Rows)
                    {
                        wt.WriteStartElement(dtbl.TableName);
                        foreach (DataColumn col in dtbl.Columns)
                        {
                            wt.WriteStartElement(col.ColumnName);

                            JSON jdat = new JSON(row[col]);
                            if (!jdat.IsNull)
                            {
                                switch (jdat.Type)
                                {
                                    case JSONType.Array:
                                    case JSONType.Object:
                                        wt.WriteValue(JSON.Tools.CleanInvalidXmlChars(jdat.ToString()));
                                        break;

                                    case JSONType.Number:
                                        wt.WriteValue(jdat.NumberValue);
                                        break;

                                    case JSONType.String:
                                        wt.WriteValue(JSON.Tools.CleanInvalidXmlChars(jdat.StringValue));
                                        break;

                                    case JSONType.Bool:
                                        wt.WriteValue(jdat.Value);
                                        break;

                                    case JSONType.Unknown:
                                        if (jdat.Value == null) wt.WriteAttributeString("IsNull", "1");
                                        else wt.WriteValue(JSON.Tools.CleanInvalidXmlChars(jdat.Value.ToString()));
                                        break;
                                }
                            }
                            else wt.WriteAttributeString("IsNull", "1");
            ...
            /**
             * Clean invalid characters for xml
             */
        ...
        public JSON this[int[] indexes, JSON def]
        {
            get
            {
                if (indexes == null) return def;
                foreach (int index in indexes)
                {
                    if (_ctype == JSONType.Array && index >= 0 && index < this.GetArrMax(false)) return ArrGet(index, JSON.Null);
                    else if (_ctype == JSONType.Object) return ObjGet(index.ToString(), def);
                }
                return def;
            }
        ...
        public JSON this[string[] keys, JSON def]
        {
            get
            {
                ...
                foreach (string key in keys)
                {
                    ...
                    {
                        int i = -1;
                        if (int.TryParse(key, out i) && i >= 0 && i < this.GetArrMax(false)) return ArrGet(i, JSON.Null);
                    }
                    else if (_ctype == JSONType.Object && this.ObjExists(key)) return ObjGet(key);
                }
                return def;
            }
        }
        ...
        public JSON ArrGet(int index)
        {
            ...
            if (index >= 0 && index < this.GetArrMax(false))
            {
        ...
        #region "Manipulate"
        public bool Append(string key, JSON val)
        {
            switch (this.Type)
            {
                case JSONType.Object:
                ...
                case JSONType.Array:
                    int i = 0;
                    if (int.TryParse(key, out i)) this[i] = val;
                    else return false;
                    break;
        ...
        public bool Append(int key, JSON val)
        {
            return this.Append(key.ToString(), val);
        }
        ...
        public bool ArrDel(int index)
        {
            if (_ctype != JSONType.Array) return false;
            int i = _cind.IndexOf(index);
            if (index >= 0 && index < this.GetArrMax(true))
            {
                if (i >= 0)
                {
                    _cind.RemoveAt(i);
        ...
        public override string ToString()
        {
            ...
            switch (_ctype)
            {
                ...
                case JSONType.String:
                    ...
                    if (_ctype == JSONType.Number) r = r.ToString().Replace(",", ".");
                    else if (_ctype == JSONType.String) r = "\"" + r + "\"";
                    break;
        ...
        public void CopyToParameter(Parameter val)
        {
            switch (val.DataType)
            {
                case SqlDbType.NChar:
                case SqlDbType.Char:
                    val.Value = (!this.IsNull && this.StringValue.Length > 0 ? (object)this.StringValue[0] : DBNull.Value);
                    break;

                case SqlDbType.Bit:
                    val.Value = (!this.IsNull ? (object)this.BoolValue : DBNull.Value);
                    break;

                case SqlDbType.Date:
                case SqlDbType.DateTime:
                    val.Value = (!this.IsNull ? this.DateTimeValue(DBNull.Value) : DBNull.Value);
                    break;
                ...
                case SqlDbType.NText:
                    val.Value = (!this.IsNull ? (object)this.StringValue : DBNull.Value);
                    break;

                case SqlDbType.TinyInt:
                    val.Value = (!this.IsNull ? (object)Int16.Parse(this.NumberValue.ToString()) : DBNull.Value);
                    break;

                case SqlDbType.Int:
                    val.Value = (!this.IsNull ? (object)int.Parse(this.NumberValue.ToString()) : DBNull.Value);
                    break;

                case SqlDbType.BigInt:
                    val.Value = (!this.IsNull ? (object)Int64.Parse(this.NumberValue.ToString()) : DBNull.Value);
                    break;
                ...
                case SqlDbType.Real:
                    val.Value = (!this.IsNull ? (object)this.NumberValue : DBNull.Value);
                    break;

                case SqlDbType.UniqueIdentifier:
                    val.Value = (!this.IsNull ? this.GuidValue(DBNull.Value) : DBNull.Value);
                    break;
        ...
        public int CopyToParameterList(List<Parameter> vals)
        {
            int count = 0;
            for (var i = 0; i < vals.Count; i++)
            {
                Parameter c = vals[i];
                string pn = c.ParameterName;
                if (pn.StartsWith("@")) pn = pn.Substring(1);

                switch (_ctype)
                {
                    case JSONType.Object:
                        if (this.ObjExists(pn))
                        {
                            this[pn].CopyToParameter(vals[i]);
                            count += 1;
                        }
                        break;
                    ...
                    case JSONType.Array:
                        if (_cchild.Count > i)
                        {
                            _cchild[i].CopyToParameter(vals[i]);
                            count += 1;
                        }
                        break;
        ...
        static public JSON Default(JSON jdat)
        {
            if (jdat == null) jdat = JSON.Null;
            return jdat;
        }
        static public JSON Default(JSON jdat, JSONType typ)
        {
            jdat = Default(jdat);
            if (jdat.Type != typ) jdat = new JSON(typ);
            return jdat;
        }
        #endregion;
