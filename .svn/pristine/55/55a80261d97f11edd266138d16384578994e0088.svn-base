...
namespace ISA.Finance.Class
{
    class PeriodeClosing
    {
        ...
        public static bool isClosingKasirOneWeek(DateTime TglClosing)
        {
            bool isClosed = true;
            try
            {
                ...
                {
                    db.Commands.Clear();
                    db.Commands.Add(db.CreateCommand("usp_KasirLog_Check"));
                    db.Commands[0].Parameters.Add(new Parameter("@Now", SqlDbType.VarChar, TglClosing));
                    dt = db.Commands[0].ExecuteDataTable();
                }
                if (dt.Rows.Count > 0)
                {
                    isClosed = false;
                    System.Windows.Forms.MessageBox.Show("Tidak bisa proses, user belum closing LKH periode sebelumnya", "Informasi", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information); //WinForms
                }
