 USE [ISAdb]
GO
/****** Object:  StoredProcedure [dbo].[rsp_Pembelian_RegisterPembelian]    Script Date: 04/25/2011 18:32:57 ******/
SET ANSI_NULLS ON
GO
...
-- =====================================================================
CREATE PROCEDURE [dbo].[rsp_Pembelian_RegisterPembelian] 
...
BEGIN
	...
	FROM dbo.KelompokBarang k
		...
		(
			SELECT
				...
				SUM( (dbo.fnHitungNet3Disc( (CASE WHEN @tipeTgl = 'NT' 
					THEN dbo.fnGetHargaBeli(nd.BarangID, nh.TglNota, NULL) ELSE nd.HPPSolo END),
					nd.Disc1, nd.Disc2, nd.Disc3, '' ) - nd.Pot)
					* (CASE WHEN @tipeTgl = 'NT' THEN nd.QtyNota ELSE nd.QtySuratJalan END) ) AS Nilai
	...
	ORDER BY nota.TglSuratJalan, nota.NoNota ASC
