...
{
    public partial class frmpilihbonusDO : ISA.Trading.BaseForm
    {
        ...
        public void promobarangBE()
        {
            //DataTable dtPromoBarang = Prolib.DataPromoBarangBonus(SumqtyBarang, SumHargaBarang);
            datagridviewBarangPromo.DataSource = dtsumKelipatanQtyBarang.DefaultView;

            for (int i = 0; i < datagridviewBarangPromo.Rows.Count; ++i)
            {
                barangid = dtsumKelipatanQtyBarang.Rows[i]["id_brg"].ToString();
                //cekstok(barangid);
                CekStokAkhir(barangid);
        ...
        public void CekStokAkhir(string barangid)
        {
            try
            {
                using (Database db = new Database())
                {
                    db.Commands.Add(db.CreateCommand("usp_CekStokAkhirGudang"));
                    db.Commands[0].Parameters.Add(new Parameter("@barangid", SqlDbType.VarChar, barangid));
        ...
        private void datagridviewBarangPromo_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //if (ix >= 0)
            //{
            //    DataGridViewCheckBoxCell checkrow = (DataGridViewCheckBoxCell)datagridviewBarangPromo.Rows[ix].Cells["pilih"];
            ...
            //    if (isChecked)
            //    {
            //        Guid rowIDPromo_ = new Guid(datagridviewBarangPromo.SelectedCells[0].OwningRow.Cells["RowID"].Value.ToString());
            //        string barangID_ = datagridviewBarangPromo.SelectedCells[0].OwningRow.Cells["IDbarang"].Value.ToString();
            //        int maxBonus_ = Convert.ToInt32(datagridviewBarangPromo.SelectedCells[0].OwningRow.Cells["MaxBonus"].Value.ToString());
            //        int JmlBns_ = 0;
            //        for (int i = 0; i < datagridviewBarangPromo.Rows.Count; i++)
            //        {
            //            if (datagridviewBarangPromo.Rows[i].Cells["RowID"].Value.ToString() == rowIDPromo_.ToString())
            //            {
            //                JmlBns_ += Convert.ToInt32(datagridviewBarangPromo.Rows[i].Cells["QtyBonus"].Value.ToString());
            //            }
            //        }

            //        if (JmlBns_ <= maxBonus_)
            //        {
            //            for (int i = 0; i < datagridviewBarangPromo.Rows.Count; i++)
            //            {
            //                if (datagridviewBarangPromo.Rows[i].Cells["RowID"].Value.ToString() == rowIDPromo_.ToString() &&
            //                    datagridviewBarangPromo.Rows[i].Cells["IDbarang"].Value.ToString() == barangID_)
            //                {
            //                    datagridviewBarangPromo.Rows[i].Cells["QtyBonus"].Value = 1;
            //                }
            //            }
            //        }
            //        else
        ...
        private void datagridviewBarangPromo_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            //kalo ngga ada count di gridnya diya balik lagi
            if (e.RowIndex < 0)
            {
                ...
                if (JmlBns_ > maxBonus_)
                {
                    for (int i = 0; i < datagridviewBarangPromo.Rows.Count; i++)
                    {
                        if (datagridviewBarangPromo.Rows[i].Cells["RowID"].Value.ToString() == rowIDPromo_.ToString() &&
                            datagridviewBarangPromo.Rows[i].Cells["IDbarang"].Value.ToString() == barangID_)
                        {
                            datagridviewBarangPromo.Rows[i].Cells["QtyBonus"].Value = 0;
                        }
                    }
                }
            }
