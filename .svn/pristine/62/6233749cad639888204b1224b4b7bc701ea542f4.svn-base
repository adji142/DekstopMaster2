USE [ISAdb]
GO
/****** Object:  StoredProcedure [dbo].[psp_SYNC_DOWNLOAD_ReturPembelianManualDetail]    Script Date: 10/11/2011 15:36:41 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[psp_SYNC_DOWNLOAD_ReturPembelianManualDetail] 
...
BEGIN	
	...
	BEGIN
		...
		IF EXISTS (SELECT RowID FROM dbo.ReturPembelianManualDetail (NOLOCK) WHERE RowID = @RowID)
			UPDATE ReturPembelianManualDetail WITH (ROWLOCK)
			SET
				RowID = @RowID,
				HeaderID = @HeaderID,				
				RecordID = @RecordID,
		...
		ELSE
			...
			BEGIN
				INSERT INTO ReturPembelianManualDetail WITH (ROWLOCK)
					(RowID, HeaderID, RecordID, ReturID, BarangID, KodeRetur, QtyGudang, QtyTerima, HrgBeli, HrgNet, HrgPokok, HPPSolo, Catatan, TglKeluar, KodeGudang, SyncFlag, LastUpdatedBy, LastUpdatedTime)
					VALUES	(
								@RowID,
								@HeaderID,							
