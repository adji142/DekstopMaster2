IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[ISA_TokoCleanUp].[dbo].[Giro]') AND type in (N'U'))
DROP TABLE ISA_TokoCleanUp.dbo.Giro
GO

declare @cabangID varchar(2)
select @cabangID = InitCabang from ISAdb.dbo.Perusahaan (nolock)

SELECT * INTO ISA_TokoCleanUp.dbo.Giro FROM ISAFinance.dbo.Giro t
...
UPDATE ISAFinance.dbo.Giro 
SET KodeToko = b.KodeTokoHeader
FROM ISAFinance.dbo.Giro a inner join isadb.dbo.mappingtoko2012 b
