USE [ISAdb]
GO
/****** Object:  StoredProcedure [dbo].[usp_OrderPenjualanDetail_LIST_BO]    Script Date: 03/22/2011 09:53:40 ******/
SET ANSI_NULLS ON
GO
...
-- ====================================================
CREATE PROCEDURE [dbo].[usp_OrderPenjualanDetail_LIST_BO] 
...
BEGIN
	...
	SELECT 
		...
		/* Hitung Qty BO = QtyDO - QtySuratJalan */
		a.QtyDO - ISNULL((SELECT SUM(f.QtySuratJalan) FROM dbo.NotaPenjualanDetail f 
			WHERE f.DODetailID = a.RowID), 0) AS QtyBO, 
	...
	WHERE
		...
		AND
		/* Filter: jika QtyDO-QtySuratJalan > 0 */
		(a.QtyDO - (SELECT SUM(n.QtySuratJalan) FROM dbo.NotaPenjualanDetail n
			WHERE a.RowID = n.DODetailID) > 0)
