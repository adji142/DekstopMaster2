IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[ISA_TokoCleanUp].[dbo].[DetailPlafon]') AND type in (N'U'))
DROP TABLE ISA_TokoCleanUp.dbo.DetailPlafon
GO

declare @cabangID varchar(2)
select @cabangID = InitCabang from ISAdb.dbo.Perusahaan (nolock)

SELECT * INTO ISA_TokoCleanUp.dbo.DetailPlafon FROM ISAdb.dbo.DetailPlafon t
...
UPDATE isadb.dbo.DetailPlafon 
SET KodeToko = b.KodeTokoHeader
FROM isadb.dbo.DetailPlafon a inner join isadb.dbo.mappingtoko2012 b
