...
select a.nokoreksi, a.RecordID, b.recordid from KoreksiPenjualan a left outer join koreksipenjualan_mdn b on a.NoKoreksi = b.nokoreksi and a.BarangID = b.barangid
where
...
select * from KoreksiPenjualan where nokoreksi = 'P/09-11/22'
select * from KoreksiPenjualan_mdn where nokoreksi = 'P/09-11/22'
select top 100 * from KoreksiPenjualan_mdn order by tglkoreksi desc
...
SELECT * FROM dbo.KoreksiPenjualan_MDN a
where
a.TglKoreksi >= '2011/08/01' --and '2011/09/30'
and a.RecordID like 'MDN%'
