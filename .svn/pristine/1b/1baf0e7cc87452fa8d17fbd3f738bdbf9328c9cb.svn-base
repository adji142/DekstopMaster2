    ...
    public partial class frmPinMd5 : Form
    {
        ...
        private void cmdYes_Click(object sender, EventArgs e)
        {
            ...
            if (ISA.Pin.key.VerifyMd5Hash(md5Hash, _pinKey, txtPin.Text))
            {
                ...
                {
                    if (_bagian != PinId.Bagian.Harga && (_bagian == PinId.Bagian.OverdueFB || _bagian == PinId.Bagian.OverdueFX))
                    {
                    ...
                    else if (_bagian == PinId.Bagian.CetakUlangNota)
                    {
                        using (Database db = new Database())
                        {
                            DataTable dt = new DataTable();
                            db.Commands.Add(db.CreateCommand("usp_NotaPenjualan_updateACC_CetakUlangNota"));
                            db.Commands[0].Parameters.Add(new Parameter("@rowID", SqlDbType.UniqueIdentifier, _rowID));
                            db.Commands[0].Parameters.Add(new Parameter("@pin", SqlDbType.VarChar, txtPin.Text));
                            db.Commands[0].Parameters.Add(new Parameter("@lastUpdatedBy", SqlDbType.VarChar, SecurityManager.UserID));
                            dt = db.Commands[0].ExecuteDataTable();
                ...
                if (this.Caller is Penjualan.frmPotonganNotaBrowse)
                {
                    if (_bagian == PinId.Bagian.Potongan)
                    {
                        using (Database db = new Database())
                        {
                            DataTable dt = new DataTable();
                            db.Commands.Add(db.CreateCommand("usp_PotonganPenjualan_UPDATE_ACC"));
                            db.Commands[0].Parameters.Add(new Parameter("@RowID", SqlDbType.UniqueIdentifier, _rowID));
                            db.Commands[0].Parameters.Add(new Parameter("@lastUpdatedBy", SqlDbType.VarChar, SecurityManager.UserID));
                            dt = db.Commands[0].ExecuteDataTable();
                            db.Commands[0].ExecuteNonQuery();
                ...
                if (this.Caller is PJ3.frmPJ3BrowserISA)
                {
