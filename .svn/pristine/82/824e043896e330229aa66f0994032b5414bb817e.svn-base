IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[ISA_TokoCleanUp].[dbo].[TokoKasus]') AND type in (N'U'))
DROP TABLE ISA_TokoCleanUp.dbo.TokoKasus
GO

declare @cabangID varchar(2)
select @cabangID = InitCabang from ISAdb.dbo.Perusahaan (nolock)

SELECT * INTO ISA_TokoCleanUp.dbo.TokoKasus FROM ISAFinance.dbo.TokoKasus t
...
UPDATE ISAFinance.dbo.TokoKasus 
SET KodeToko = b.KodeTokoHeader
FROM ISAFinance.dbo.TokoKasus a inner join isadb.dbo.mappingtoko2012 b
