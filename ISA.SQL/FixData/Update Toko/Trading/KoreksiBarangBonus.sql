IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[ISA_TokoCleanUp].[dbo].[BarangBonus]') AND type in (N'U'))
DROP TABLE ISA_TokoCleanUp.dbo.BarangBonus
GO

declare @cabangID varchar(2)
select @cabangID = InitCabang from ISAdb.dbo.Perusahaan (nolock)

SELECT * INTO ISA_TokoCleanUp.dbo.BarangBonus FROM ISAdb.dbo.BarangBonus t 
...
UPDATE isadb.dbo.BarangBonus 
SET KodeToko = b.KodeTokoHeader
FROM isadb.dbo.BarangBonus a inner join isadb.dbo.mappingtoko2012 b
