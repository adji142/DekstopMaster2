IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[ISA_TokoCleanUp].[dbo].[AccountToko]') AND type in (N'U'))
DROP TABLE ISA_TokoCleanUp.dbo.AccountToko
GO

declare @cabangID varchar(2)
select @cabangID = InitCabang from ISAdb.dbo.Perusahaan (nolock)

SELECT * INTO ISA_TokoCleanUp.dbo.AccountToko FROM ISAFinance.dbo.AccountToko t
...
UPDATE ISAFinance.dbo.AccountToko 
SET KodeToko = b.KodeTokoHeader
FROM ISAFinance.dbo.AccountToko a inner join isadb.dbo.mappingtoko2012 b
