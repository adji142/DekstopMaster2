...
IF OBJECT_ID('[dbo].[rsp_PenyimpanganACC]') IS NOT NULL
DROP PROC [dbo].[rsp_PenyimpanganACC] 
GO

/****** Object:  StoredProcedure [dbo].[rsp_PenyimpanganACC]    Script Date: 03/09/2011 10:44:35 ******/
SET ANSI_NULLS ON
GO
...
-- Description:	VACCDO > Laporan > Penyimpangan ACC
-- ============================================================
CREATE PROCEDURE [dbo].[rsp_PenyimpanganACC] 
...
SELECT
	...
	WHERE	
		((x.RpNet>(x.Plafon-x.SaldoPiutang)) OR x.Overdue <> 0)
		AND ((LEFT(x.NoACCPiutang, 1) = 'S') OR (LEFT(x.NoACCPiutang, 1) = 'F'))			
		
	ORDER BY x.TglDO ASC
